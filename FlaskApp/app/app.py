...
def process_features(final_df):
    ...
    final_df['carrier_popularity'] = final_df['OP_CARRIER'].apply(
        lambda x: label_carrier_train(x))
...
def predict():
    ...
    if len(str(departure_time_object.minute)) == 2:
        DEP_TIME = int(str(departure_time_object.hour) +
                       str(departure_time_object.minute))
    elif len(str(departure_time_object.minute)) == 1:
        DEP_TIME = int(str(departure_time_object.hour) +
                       '0'+str(departure_time_object.minute))
    else:
    ...
    if len(str(arrival_time_object.minute)) == 2:
        ARR_TIME = int(str(arrival_time_object.hour) +
                       str(arrival_time_object.minute))
    elif len(str(arrival_time_object.minute)) == 1:
        ARR_TIME = int(str(arrival_time_object.hour) +
                       '0'+str(arrival_time_object.minute))
    else:
        ARR_TIME = int(str(arrival_time_object.hour)+'00')
    is_weekend = 0
    if DAY_OF_WEEK == 5 or DAY_OF_WEEK == 6:
        is_weekend = 1
    features = [month, DAY_OF_MONTH, OP_CARRIER, ORIGIN, DEST,
                DEP_TIME, ARR_TIME, DAY_OF_WEEK, car_popularity, is_weekend]
